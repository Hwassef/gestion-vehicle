...
<body>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button type="submit" class="btn btn-success openApprovalReservation" data-toggle="modal" data-target="#approve" data-id="{{$reservation -> id}}">Approve</button>
                        <button type="submit" class="btn btn-warning openDisapproveReservation" data-toggle="modal" data-target="#disapprove" data-id="{{$reservation -> id}}">Disapprove</button>
                    </td>
                    @endforeach
                </tr>
            </tbody>
    ...
    <div id="approve" class="modal fade">
        <div class="modal-dialog modal-confirm">
            <div class="modal-content">
                <form action="{{route('admin.approveReservation')}}" method="post">
                    @csrf
                    <div class="modal-header">
                        <div class="icon-box" style="color: #82CE34;border: 3px solid #82CE34;">
                            <i class="fas fa-check-circle " style="color: #82CE34; font-size:5em;"></i>
                        </div>
                    </div>
                    <h4 class="modal-title">Confirm Approval</h4>
                    <input type="hidden" name="confirmReservationId" id="confirmReservationId">
                    <input type="hidden" name="reservationUserId" id="reservationUserId">
                    <div class="modal-body">
                        <p class="text-center">You are about to approve the reservation for:</p>
                        <div class="d-flex justify-content-start">
                            <div class="col">
                                Full Name: <span id="cofirmReservationClientName"></span>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col">
                                Vehicle Brand: <span id="cofirmReservationVehicleBrand"></span>
                            </div>
                        </div>
                    </div>
    ...
    <div id="disapprove" class="modal fade">
        <div class="modal-dialog modal-confirm">
            <div class="modal-content">
                <form action="{{route('admin.disapproveReservation')}}" method="post">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <input type="hidden" name="disapproveReservationId" id="disapproveReservationId">
                        <p>Do you really want to disapprove the reservation for :</p>
                        <div class="d-flex justify-content-start">
                            <div class="col">
                                Full Name: <span id="disapproveClientName"></span>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col">
                                Vehicle Brand: <span id="disapproveVehicleBrand"></span>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer d-flex justify-content-center">
                        <button type="button" class="btn btn-info" data-dismiss="modal">Cancel</button>
                        <button class="btn btn-danger" type="submit">Disaprove</button>
    ...
    <div id="reservationInfo" class="modal fade">
        ...
        <script>
            $(document).on('click', '.openApprovalReservation', function() {
                var vehicle_brand = $(this).closest("tr").find('td.vehicle_brand').text();
                var client_name = $(this).closest("tr").find('td.client_name').text();
                var id = $(this).closest("tr").find('td.id').text();
                var user_id = $(this).closest("tr").find('td.user_id').text();
                $('#confirmReservationId').val(id);
                $("#reservationUserId").val(user_id);

                $(document).ready(function() {
                    $("#cofirmReservationVehicleBrand").text(vehicle_brand);
                    $("#cofirmReservationClientName").text(client_name);
                });
            });
        </script>
        ...
        <script>
            $(document).on('click', '.openDisapproveReservation', function() {
                var vehicle_brand = $(this).closest("tr").find('td.vehicle_brand').text();
                var client_name = $(this).closest("tr").find('td.client_name').text();

                var id = $(this).closest("tr").find('td.id').text();
                $('#disapproveReservationId').val(id);

                $(document).ready(function() {
                    $("#disapproveVehicleBrand").text(vehicle_brand);
                    $("#disapproveClientName").text(client_name);
                });
            });
        </script>
</body>
